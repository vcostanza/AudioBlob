package software.blob.audio.wave;// Wav file IO class
// A.Greensted
...
public class WavFile
{
	...
	private final static int COMPRESSION_DEFAULT = 1;
	private final static int COMPRESSION_FLT = 3;
	...
	private int compressionCode;		// Compression code (default = 1)
	...
	public static WavFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		while (true)
		{
			...
			if (chunkID == FMT_CHUNK_ID)
			{
				...
				wavFile.compressionCode = (int) getLE(wavFile.buffer, 0, 2);
				if (wavFile.compressionCode != COMPRESSION_DEFAULT && wavFile.compressionCode != COMPRESSION_FLT)
					throw new WavFileException("Compression Code " + wavFile.compressionCode + " not supported");
	...
	public int readFrames(double[][] sampleBuffer, int offset, int numFramesToRead) throws IOException, WavFileException
	{
		...
		for (int f=0 ; f<numFramesToRead ; f++)
		{
			...
			for (int c=0 ; c<numChannels ; c++) {
				...
				if (compressionCode == COMPRESSION_FLT)
					sampleBuffer[c][offset] = Float.intBitsToFloat((int) sample);
				else
					sampleBuffer[c][offset] = floatOffset + (double) sample / floatScale;
			}
